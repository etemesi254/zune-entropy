...
fn criterion_benchmark(c: &mut Criterion)
{
    ...
    let mut reader = std::io::Cursor::new(std::fs::read("/Users/calebe/git/FiniteStateEntropy/programs/enwiki.small").unwrap());



    c.bench_function("Huff Compress 4X", |b| {
        b.iter(|| black_box(huff_compress_4x(&mut reader, &mut writer)));
